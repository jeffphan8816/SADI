...
public class SimpleTestClient
{
	...
	/*	for (Player player : players)
		{
			gameEngine.placeBet(player, 700);
		...
		for (Player player : players) {
			if (gameEngine.placeBet(player, 100)==true) {
				gameEngine.addPlayer(player);
				gameEngine.rollPlayer(player, 1, 100, 20);
			}
		}
		...
		//Collection<Player> gameEngineCollection = new ArrayList<Player>(gameEngine.getAllPlayers());
		
//		for (Player player : gameEngine.getAllPlayers()) {
//			System.out.printf("from gameEngine PLayer name: %s\n", player.getPlayerName());
//		}
//	
//		for (Player player : players) {
//			System.out.printf("Main PLayer name: %s\n", player.getPlayerName());
//		}
	}
}
